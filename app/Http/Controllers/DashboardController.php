...
//        dd($hirer->id);
        ...
        if($hirer != null) {
            $hirerResources = HirerResource::where(['hirer_id' => $hirer->id, 'completed' => false])->orderBy('created_at', 'DESC')->get();
            $oldHirerResources = HirerResource::where(['hirer_id' => $hirer->id, 'completed' => true])->orderBy('created_at', 'DESC')->get();
        }
        ...
        if ($hirerResources == null and $resource->count() == 1)
        {
            ...
            $hirerResources = HirerResource::where(['resource_id' => $resource->id, 'completed' => false])->orderBy('created_at', 'DESC')->get();
